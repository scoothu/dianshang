...
针对电商后台的登录、商品、订单三大业务模块，搭建 Flask Mock 测试服务，实现 15+ 核心接口，构建独立隔离测试环境；并基于 Python + Pytest + Requests 编写全量接口自动化脚本，完成端到端全链路自动化（UI+API+DB 三层联调）。同时使用 HTML/CSS/JS 开发 Web 可视化管理界面，实现从业务模拟 → 接口测试 → 可视化演示的完整闭环。

## 技术栈

**Python, Pytest, Requests, Flask, Postman, MySQL, HTML/CSS/JavaScript**
...
├── utils/                  # 工具支撑层
│   ├── config.py           # 配置管理（单例模式）
│   ├── database.py         # 数据库操作封装（支持SQLite/MySQL双模式）
...
```
UI层（Web界面）  ──操作──→  API层（Flask接口）  ──同步写入──→  DB层（SQLite/MySQL）
                                    ↓                              ↑
                              测试用例调用                     测试用例校验
                                    └──────── 一致性断言 ─────────┘
```

- **UI层**：Web 可视化管理界面，模拟真实用户操作
- **API层**：Flask Mock 服务提供 15+ 核心接口，内存+数据库双写
- **DB层**：测试用例调用 API 后，直接查询数据库验证数据一致性，确保业务数据正确落库
...
- **独立隔离测试环境**：搭建 Flask Mock 测试服务，实现登录、商品、订单 15+ 核心接口，数据双写（内存 + SQLite 数据库）
- **UI+API+DB 三层联调**：测试用例同时校验接口响应与数据库一致性，确保业务数据正确落库
- **全量自动化测试**：基于 Python + Pytest + Requests 编写全量接口自动化脚本，实现断言封装、日志记录、参数化驱动
- **用例覆盖**：梳理 3 大核心模块（登录/商品/订单）业务规则，设计 47 条功能测试用例，覆盖正常/边界/异常输入场景，**全部通过**
- **自动化回归**：核心接口自动化回归覆盖率达 **85%+**，回归测试效率提升约 **60%**
- **缺陷闭环**：发现并修复「登录模块用户名长度校验顺序异常」缺陷，通过回归测试验证修复后关闭
- **交付物**：生成 HTML 测试报告 + PDF 文档，形成完整项目交付物
