...
针对电商后台的登录、商品、订单三大业务模块，搭建 Flask Mock 测试服务，实现 15+ 核心接口，构建独立隔离测试环境；并基于 Python + Pytest + Requests 编写全量接口自动化脚本，完成端到端全链路自动化测试。同时使用 HTML/CSS/JS 开发 Web 可视化管理界面，实现从业务模拟 → 接口测试 → 可视化演示的完整闭环。
...
- **独立隔离测试环境**：搭建 Flask Mock 测试服务，实现登录、商品、订单 15+ 核心接口
